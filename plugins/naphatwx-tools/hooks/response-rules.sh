...
case "${1:-}" in
    session)
        ...
        ;;
    prompt)
        echo 'RESPONSE RULES ACTIVE. Format this reply: lists only (no prose paragraphs); headers are plain bold lines, never list items; an `&nbsp;` line between every block, never between bullets of one list. Chat only, never in files.'
        ;;
esac
exit 0
